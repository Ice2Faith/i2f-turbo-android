...
public class AnimatePaintView extends View {
    ...
    private void ParseAttrs(AttributeSet attrs) {
        ...
        this.ballCount =ta.getInt(R.styleable.AnimatePaintView_ani_ball_count,60);
    }
            ...
            public void run() {
                ...
                for (int i = 0; i < ballCount; i++) {
                    ...
                    if (ball.x <= 0 || ball.x >= wid
                        ...
                        double faltRadian=0;
                        if(ball.x<0 || ball.x>=wid){
                            faltRadian= PhysicalReflect.AngleToRadian(90);
                        }
                        ball.direct=PhysicalReflect.getReflectRadian(ball.direct,
                                faltRadian);
                    }
                }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for(int i = 0; i< ballCount; i++){
            Ball ball=new Ball(rand.nextInt(w),
                    rand.nextInt(h),
                    rand.nextInt(25)+5);
            allBalls.add(ball);
        }
