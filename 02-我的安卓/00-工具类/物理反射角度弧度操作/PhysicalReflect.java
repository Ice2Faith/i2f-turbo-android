...
public class PhysicalReflect {
	public static void main(String ... args) {
		Point sPoint=new Point(10, 40);
		double length=15;
		double direct=getRandomRadian();
		...
		double outAngle=RadianToAlgle(getReflectRadian(AngleToRadian(inAngle), AngleToRadian(flatAngle)));
		double noutAngle=RadianToAlgle(getReflectRadian(AngleToRadian(inAngle), AngleToRadian(nflatAngle)));
		System.out.println(inAngle+" V "+flatAngle+" ==> "+outAngle);
		System.out.println(inAngle+" V "+nflatAngle+" ==> "+noutAngle);
	}
    /**
     * 计算反射角（弧度计算）
     * @param inRadian 入射弧度
    ...
    public static double getRegularAngle(double angle){
        int limitControl=1000; //精度控制
        double result=((int)angle*limitControl)%(360*limitControl)*1.0/limitControl;
        if(result<0)
        	result+=360.0;
        return result;
    }
